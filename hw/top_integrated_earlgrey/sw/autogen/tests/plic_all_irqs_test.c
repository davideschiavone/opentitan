...
static const top_integrated_earlgrey_plic_target_t kHart = kTopIntegrated_earlgreyPlicTargetIbex0;
 ...
 * as the ISR.
 */
static volatile top_integrated_earlgrey_plic_peripheral_t peripheral_expected;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
static volatile dif_mailbox_irq_t mailbox_irq_expected;
static volatile dif_mailbox_irq_t mailbox_irq_serviced;
#endif
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  top_integrated_earlgrey_plic_peripheral_t peripheral = (top_integrated_earlgrey_plic_peripheral_t)
      top_integrated_earlgrey_plic_interrupt_for_peripheral[plic_irq_id];
  ...
  switch (peripheral) {
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralAdcCtrlAon: {
      dif_adc_ctrl_irq_t irq =
          (dif_adc_ctrl_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopIntegratedEarlgreyPlicIrqIdAdcCtrlAonMatchPending);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralAlertHandler: {
      dif_alert_handler_irq_t irq =
          (dif_alert_handler_irq_t)(plic_irq_id -
                                    (dif_rv_plic_irq_id_t)
                                        kTopIntegratedEarlgreyPlicIrqIdAlertHandlerClassa);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralAonTimerAon: {
      dif_aon_timer_irq_t irq =
          (dif_aon_timer_irq_t)(plic_irq_id -
                                (dif_rv_plic_irq_id_t)
                                    kTopIntegratedEarlgreyPlicIrqIdAonTimerAonWkupTimerExpired);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralCsrng: {
      dif_csrng_irq_t irq =
          (dif_csrng_irq_t)(plic_irq_id -
                            (dif_rv_plic_irq_id_t)
                                kTopIntegratedEarlgreyPlicIrqIdCsrngCsCmdReqDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralEdn0: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopIntegratedEarlgreyPlicIrqIdEdn0EdnCmdReqDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralEdn1: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopIntegratedEarlgreyPlicIrqIdEdn1EdnCmdReqDone);
      CHECK(irq == edn_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralEntropySrc: {
      dif_entropy_src_irq_t irq =
          (dif_entropy_src_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopIntegratedEarlgreyPlicIrqIdEntropySrcEsEntropyValid);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralFlashCtrl: {
      dif_flash_ctrl_irq_t irq =
          (dif_flash_ctrl_irq_t)(plic_irq_id -
                                 (dif_rv_plic_irq_id_t)
                                     kTopIntegratedEarlgreyPlicIrqIdFlashCtrlProgEmpty);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralGpio: {
      dif_gpio_irq_t irq =
          (dif_gpio_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdGpioGpio0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralHmac: {
      dif_hmac_irq_t irq =
          (dif_hmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdHmacHmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralI2c0: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopIntegratedEarlgreyPlicIrqIdI2c0FmtThreshold);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralI2c1: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopIntegratedEarlgreyPlicIrqIdI2c1FmtThreshold);
      CHECK(irq == i2c_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralI2c2: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopIntegratedEarlgreyPlicIrqIdI2c2FmtThreshold);
      CHECK(irq == i2c_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralKeymgr: {
      dif_keymgr_irq_t irq =
          (dif_keymgr_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopIntegratedEarlgreyPlicIrqIdKeymgrOpDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralKmac: {
      dif_kmac_irq_t irq =
          (dif_kmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdKmacKmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralMailbox: {
      dif_mailbox_irq_t irq =
          (dif_mailbox_irq_t)(plic_irq_id -
                              (dif_rv_plic_irq_id_t)
                                  kTopIntegratedEarlgreyPlicIrqIdMailboxDoorbell);
      CHECK(irq == mailbox_irq_expected,
            "Incorrect mailbox IRQ triggered: exp = %d, obs = %d",
            mailbox_irq_expected, irq);
      mailbox_irq_serviced = irq;

      dif_mailbox_irq_state_snapshot_t snapshot;
      CHECK_DIF_OK(dif_mailbox_irq_get_state(&mailbox, &snapshot));
      CHECK(snapshot == (dif_mailbox_irq_state_snapshot_t)(1 << irq),
            "Only mailbox IRQ %d expected to fire. Actual interrupt "
            "status = %x",
            irq, snapshot);
      ...
      if (0x3 & (1 << irq)) {
        // We do not acknowledge status type interrupt at the IP side, but we
        // need to clear the test force register.
        CHECK_DIF_OK(dif_mailbox_irq_force(&mailbox, irq, false));
        // In case this status interrupt is asserted by default, we also
        // disable it at this point so that it does not interfere with the
        // rest of the test.
        if ((0x0 & (1 << irq))) {
          CHECK_DIF_OK(dif_mailbox_irq_set_enabled(&mailbox, irq, false));
        }
      } else {
        // We acknowledge event type interrupt.
        CHECK_DIF_OK(dif_mailbox_irq_acknowledge(&mailbox, irq));
      }
      break;
    }
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralOtbn: {
      dif_otbn_irq_t irq =
          (dif_otbn_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdOtbnDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralOtpCtrl: {
      dif_otp_ctrl_irq_t irq =
          (dif_otp_ctrl_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopIntegratedEarlgreyPlicIrqIdOtpCtrlOtpOperationDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralPattgen: {
      dif_pattgen_irq_t irq =
          (dif_pattgen_irq_t)(plic_irq_id -
                              (dif_rv_plic_irq_id_t)
                                  kTopIntegratedEarlgreyPlicIrqIdPattgenDoneCh0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralPwrmgrAon: {
      dif_pwrmgr_irq_t irq =
          (dif_pwrmgr_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopIntegratedEarlgreyPlicIrqIdPwrmgrAonWakeup);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralRvTimer: {
      dif_rv_timer_irq_t irq =
          (dif_rv_timer_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopIntegratedEarlgreyPlicIrqIdRvTimerTimerExpiredHart0Timer0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralSensorCtrlAon: {
      dif_sensor_ctrl_irq_t irq =
          (dif_sensor_ctrl_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopIntegratedEarlgreyPlicIrqIdSensorCtrlAonIoStatusChange);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralSpiDevice: {
      dif_spi_device_irq_t irq =
          (dif_spi_device_irq_t)(plic_irq_id -
                                 (dif_rv_plic_irq_id_t)
                                     kTopIntegratedEarlgreyPlicIrqIdSpiDeviceUploadCmdfifoNotEmpty);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralSpiHost0: {
      dif_spi_host_irq_t irq =
          (dif_spi_host_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopIntegratedEarlgreyPlicIrqIdSpiHost0Error);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralSpiHost1: {
      dif_spi_host_irq_t irq =
          (dif_spi_host_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopIntegratedEarlgreyPlicIrqIdSpiHost1Error);
      CHECK(irq == spi_host_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralSysrstCtrlAon: {
      dif_sysrst_ctrl_irq_t irq =
          (dif_sysrst_ctrl_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopIntegratedEarlgreyPlicIrqIdSysrstCtrlAonEventDetected);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralUart0: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdUart0TxWatermark);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralUart1: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdUart1TxWatermark);
      CHECK(irq == uart_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralUart2: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdUart2TxWatermark);
      CHECK(irq == uart_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralUart3: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopIntegratedEarlgreyPlicIrqIdUart3TxWatermark);
      CHECK(irq == uart_irq_expected,
...
#if TEST_MIN_IRQ_PERIPHERAL <= 23 && 23 < TEST_MAX_IRQ_PERIPHERAL
    case kTopIntegratedEarlgreyPlicPeripheralUsbdev: {
      dif_usbdev_irq_t irq =
          (dif_usbdev_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopIntegratedEarlgreyPlicIrqIdUsbdevPktReceived);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_ADC_CTRL_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_adc_ctrl_init(base_addr, &adc_ctrl_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_ALERT_HANDLER_BASE_ADDR);
  CHECK_DIF_OK(dif_alert_handler_init(base_addr, &alert_handler));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_AON_TIMER_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_aon_timer_init(base_addr, &aon_timer_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_CSRNG_BASE_ADDR);
  CHECK_DIF_OK(dif_csrng_init(base_addr, &csrng));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_EDN0_BASE_ADDR);
  CHECK_DIF_OK(dif_edn_init(base_addr, &edn0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_EDN1_BASE_ADDR);
  CHECK_DIF_OK(dif_edn_init(base_addr, &edn1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_ENTROPY_SRC_BASE_ADDR);
  CHECK_DIF_OK(dif_entropy_src_init(base_addr, &entropy_src));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_FLASH_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_flash_ctrl_init(base_addr, &flash_ctrl));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_GPIO_BASE_ADDR);
  CHECK_DIF_OK(dif_gpio_init(base_addr, &gpio));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_HMAC_BASE_ADDR);
  CHECK_DIF_OK(dif_hmac_init(base_addr, &hmac));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_I2C0_BASE_ADDR);
  CHECK_DIF_OK(dif_i2c_init(base_addr, &i2c0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_I2C1_BASE_ADDR);
  CHECK_DIF_OK(dif_i2c_init(base_addr, &i2c1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_I2C2_BASE_ADDR);
  CHECK_DIF_OK(dif_i2c_init(base_addr, &i2c2));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_KEYMGR_BASE_ADDR);
  CHECK_DIF_OK(dif_keymgr_init(base_addr, &keymgr));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_KMAC_BASE_ADDR);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_OTP_CTRL_CORE_BASE_ADDR);
  CHECK_DIF_OK(dif_otp_ctrl_init(base_addr, &otp_ctrl));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_PATTGEN_BASE_ADDR);
  CHECK_DIF_OK(dif_pattgen_init(base_addr, &pattgen));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_PWRMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pwrmgr_init(base_addr, &pwrmgr_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_RV_TIMER_BASE_ADDR);
  CHECK_DIF_OK(dif_rv_timer_init(base_addr, &rv_timer));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SENSOR_CTRL_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_sensor_ctrl_init(base_addr, &sensor_ctrl_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SPI_DEVICE_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_device_init(base_addr, &spi_device));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SPI_HOST0_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_host_init(base_addr, &spi_host0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SPI_HOST1_BASE_ADDR);
  CHECK_DIF_OK(dif_spi_host_init(base_addr, &spi_host1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SYSRST_CTRL_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_sysrst_ctrl_init(base_addr, &sysrst_ctrl_aon));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_UART0_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart0));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_UART1_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart1));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_UART2_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart2));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_UART3_BASE_ADDR);
  CHECK_DIF_OK(dif_uart_init(base_addr, &uart3));
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 23 && 23 < TEST_MAX_IRQ_PERIPHERAL
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_USBDEV_BASE_ADDR);
  CHECK_DIF_OK(dif_usbdev_init(base_addr, &usbdev));
#endif

  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_RV_PLIC_BASE_ADDR);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_mailbox_irq_acknowledge_all(&mailbox));
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  dif_mailbox_irq_state_snapshot_t mailbox_irqs =
      (dif_mailbox_irq_state_snapshot_t)0xffffffff;
#endif
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  CHECK_DIF_OK(dif_mailbox_irq_restore_all(&mailbox, &mailbox_irqs));
#endif
...
static void peripheral_irqs_trigger(void) {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralAdcCtrlAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralAlertHandler;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralAonTimerAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralCsrng;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralEdn0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralEdn1;
  for (dif_edn_irq_t irq = kDifEdnIrqEdnCmdReqDone; irq <= kDifEdnIrqEdnFatalErr;
       ++irq) {
    edn_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralEntropySrc;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralFlashCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralGpio;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralHmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralI2c0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralI2c1;
  status_default_mask = 0x0;
  for (dif_i2c_irq_t irq = kDifI2cIrqFmtThreshold; irq <= kDifI2cIrqHostTimeout;
       ++irq) {
    i2c_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralI2c2;
  status_default_mask = 0x0;
  for (dif_i2c_irq_t irq = kDifI2cIrqFmtThreshold; irq <= kDifI2cIrqHostTimeout;
       ++irq) {
    i2c_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralKeymgr;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralKmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralMailbox;
  status_default_mask = 0x0;
  for (dif_mailbox_irq_t irq = kDifMailboxIrqDoorbell; irq <= kDifMailboxIrqCompletion;
       ++irq) {
    mailbox_irq_expected = irq;
    LOG_INFO("Triggering mailbox IRQ %d.", irq);
    CHECK_DIF_OK(dif_mailbox_irq_force(&mailbox, irq, true));

    // In this case, the interrupt has not been enabled yet because that would
    // interfere with testing other interrupts. We enable it here and let the
    // interrupt handler disable it again.
    if ((status_default_mask & 0x1)) {
      CHECK_DIF_OK(dif_mailbox_irq_set_enabled(&mailbox, irq, true));
    }
    status_default_mask >>= 1;

    // This avoids a race where *irq_serviced is read before
    // entering the ISR.
    IBEX_SPIN_FOR(mailbox_irq_serviced == irq, 1);
    LOG_INFO("IRQ %d from mailbox is serviced.", irq);
  }
#endif

#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralOtbn;
  ...
  if (kBootStage != kBootStageOwner) {
    peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralOtpCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralPattgen;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralPwrmgrAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralRvTimer;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 18 && 18 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralSensorCtrlAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 19 && 19 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralSpiDevice;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralSpiHost0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 20 && 20 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralSpiHost1;
  status_default_mask = 0x0;
  for (dif_spi_host_irq_t irq = kDifSpiHostIrqError; irq <= kDifSpiHostIrqSpiEvent;
       ++irq) {
    spi_host_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 21 && 21 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralSysrstCtrlAon;
  status_default_mask = 0x0;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralUart0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralUart1;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralUart2;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 22 && 22 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralUart3;
  status_default_mask = 0x101;
  for (dif_uart_irq_t irq = kDifUartIrqTxWatermark; irq <= kDifUartIrqTxEmpty;
       ++irq) {
    uart_irq_expected = irq;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 23 && 23 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopIntegratedEarlgreyPlicPeripheralUsbdev;
  status_default_mask = 0x0;
...
bool test_main(void) {
  ...
  rv_plic_testutils_irq_range_enable(
      &plic, kHart, kTopIntegrated_earlgreyPlicIrqIdNone + 1, kTopIntegrated_earlgreyPlicIrqIdLast);
