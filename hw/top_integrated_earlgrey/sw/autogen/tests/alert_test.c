...
static void init_peripherals(void) {
  ...
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_AES_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_CLKMGR_AON_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_PINMUX_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_pinmux_init(base_addr, &pinmux_aon));

  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_PWM_AON_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_ROM_CTRL_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_rom_ctrl_init(base_addr, &rom_ctrl));

  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_RSTMGR_AON_BASE_ADDR);
  CHECK_DIF_OK(dif_rstmgr_init(base_addr, &rstmgr_aon));

  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_RV_CORE_IBEX_CFG_BASE_ADDR);
  ...
  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SRAM_CTRL_MAIN_REGS_BASE_ADDR);
  CHECK_DIF_OK(dif_sram_ctrl_init(base_addr, &sram_ctrl_main));

  base_addr = mmio_region_from_addr(TOP_INTEGRATED_EARLGREY_SRAM_CTRL_RET_AON_REGS_BASE_ADDR);
...
static void trigger_alert_test(void) {
  ...
  for (dif_adc_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    exp_alert = kTopIntegratedEarlgreyAlertIdAdcCtrlAonFatalFault + i;
  ...
  for (dif_aes_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdAesRecovCtrlUpdateErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_aon_timer_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdAonTimerAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_clkmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdClkmgrAonRecovFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_csrng_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdCsrngRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdEdn0RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_edn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdEdn1RecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_entropy_src_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdEntropySrcRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_flash_ctrl_alert_t i = 0; i < 5; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdFlashCtrlRecovErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_gpio_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdGpioFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_hmac_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdHmacFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_i2c_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdI2c0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_i2c_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdI2c1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_i2c_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdI2c2FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_keymgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdKeymgrRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_kmac_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdKmacRecovOperationErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_otbn_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdOtbnFatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  if (kBootStage != kBootStageOwner) {
    ...
    for (dif_otp_ctrl_alert_t i = 0; i < 5; ++i) {
      ...
      // Verify that alert handler received it.
      exp_alert = kTopIntegratedEarlgreyAlertIdOtpCtrlFatalMacroError + i;
      CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
          &alert_handler, exp_alert, &is_cause));
      CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pattgen_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdPattgenFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pinmux_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdPinmuxAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pwm_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdPwmAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_pwrmgr_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdPwrmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rom_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdRomCtrlFatal + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rstmgr_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdRstmgrAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_core_ibex_alert_t i = 0; i < 4; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdRvCoreIbexFatalSwErr + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_plic_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdRvPlicFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_rv_timer_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdRvTimerFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sensor_ctrl_alert_t i = 0; i < 2; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSensorCtrlAonRecovAlert + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_device_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSpiDeviceFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_host_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSpiHost0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_spi_host_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSpiHost1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSramCtrlMainFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sram_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSramCtrlRetAonFatalError + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_sysrst_ctrl_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdSysrstCtrlAonFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdUart0FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdUart1FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdUart2FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_uart_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdUart3FatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
  ...
  for (dif_usbdev_alert_t i = 0; i < 1; ++i) {
    ...
    // Verify that alert handler received it.
    exp_alert = kTopIntegratedEarlgreyAlertIdUsbdevFatalFault + i;
    CHECK_DIF_OK(dif_alert_handler_alert_is_cause(
        &alert_handler, exp_alert, &is_cause));
    CHECK(is_cause, "Expect alert %d!", exp_alert);
