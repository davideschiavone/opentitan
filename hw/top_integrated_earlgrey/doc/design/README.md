...
`ast` supplies a number of clocks into `top_integrated_earlgrey`.
...
`ast` also supplies a number of power-okay signals to `top_integrated_earlgrey`, and these are used as asynchronous root resets.
- vcaon_pok: The always on domain of the system is ready.
- vcmain_pok: The main operating domain of the system is ready.

When one of these power-okay signals drop, the corresponding domain in `top_integrated_earlgrey` is reset.
...
While the ast supplies clocks and resets to the `top_integrated_earlgrey`, it also contains additional functions that interact with the design.
...
| AST Port         | `top_integrated_earlgrey` Clock     |
|------------------|--------------------------|
| clk_ast_adc_i    | aon                      |
...
| AST Port         | `top_integrated_earlgrey` Power Domain |
|------------------|-----------------------------|
...
Since `top_integrated_earlgrey` contains flash, it is important to examine the memory's relationship with resets.
...
#### Topology
`top_integrated_earlgrey` has a two-level hierarchy for its bus network.
...
For the purpose of `top_integrated_earlgrey`, the first option has been chosen to benefit software development and testing

<!-- BEGIN CMDGEN util/design/gen-top-docs.py -t hw/top_integrated_earlgrey/data/autogen/top_integrated_earlgrey.gen.hjson -g mmap -->
...
`top_integrated_earlgrey` has two [EDN](../../../ip/edn/README.md) instances, which are connected to one [CSRNG](../../../ip/csrng/README.md) instance (which in turn is connected to one [Entropy Source](../../../ip/entropy_src/README.md) instance).
...
<!-- BEGIN CMDGEN util/design/gen-top-docs.py -t hw/top_integrated_earlgrey/data/autogen/top_integrated_earlgrey.gen.hjson -g pinout -->
...
There is a script for this auto-generation, centered around the top-level descriptor file `top_integrated_earlgrey.hjson` found in the repository.
...
There is netlist automation to create the module `top_integrated_earlgrey` as indicated in sections of this specification that follow.
